...
| **[Delivery, DEVOPS]** DISCUSS/DESIGN assumed the `InkSwift` JS-bridge is "Apple-only … Linux CI does not build this target." | JXKit **3.6.0** (bumped for Linux dependency resolution) ships a Linux JS backend, so the JS-bridge **does** build and test on Linux; it is deliberately **kept** (not excluded) so existing Linux consumers of the `InkSwift` product are not broken. Its `libjavascriptcoregtk-4.1-dev` system dep is installed in the `test-linux` CI step. Only Combine-specific tests stay macOS-only. | The committed-fixture oracle (ADR-014) remains the *primary* cross-platform parity gate; the JS-bridge on Linux is additional coverage, not the ground truth. Not breaking downstream Linux consumers outweighed excluding the bridge. |
...
| `.swiftlint.yml` R3 rule | `.swiftlint.yml` | R3 boundary regex | **EXTEND (delivered)** | **boundary enforcement** · universe: static-analysis rule confining Ink-format JSON decoding to `Decoder/`. | Shipped as `r3_jsondecoder_boundary` (commit 0e3c3a0): binds `JSONDecoder` in `Facade/`+`Compiler/`, leaving the `Engine/` StoryState save/restore (ADR-003) permitted. |
...
| DD-5 | R3 generalized to "Ink-format JSON decoding confined to `Decoder/`"; shipped as `.swiftlint.yml` `r3_jsondecoder_boundary` (commit 0e3c3a0), scoped to avoid the ADR-003 StoryState save/restore false positive in `Engine/`. | ADR-013 |
...
| R3 enforcement regex | `.swiftlint.yml` | EXTEND (delivered — `r3_jsondecoder_boundary`) |
