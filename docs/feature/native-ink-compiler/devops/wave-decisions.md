...
|---|---|---|---|
| D1 | **Extend the existing Forgejo workflow**, macOS-only. No new CI platform; no Linux oracle job. | Existing-infrastructure-first. The execution-equivalence oracle (KPI #1) needs the macOS-only InkSwift JS-bridge, so macOS is the supported CI target. | `.forgejo/workflows/tests.yml`; user CI-scope decision |
| D2 | **Author `.swiftlint.yml` now** with path-scoped `custom_rules` for R1/R3/R5; wire the SwiftLint **stage** into `tests.yml` alongside the FIRST `Compiler/` slice in DELIVER. | The config is safe today (R5 matches nothing; R3 passes). Adding a live lint stage before swiftlint is guaranteed on the runner would risk redding green CI. Decouple config (now) from stage activation (with first slice). | brief.md R1/R3/R5; user CAUTION on green CI |
...
| D4 | **Oracle harness reuse**: committed `.ink.json` fixtures (generated offline via inklecate, REGEN-gated) replayed on macos-arm64 through the InkSwift JS-bridge. inklecate is **not** a CI runtime dependency. | Reuses the established Milestone5b oracle pattern; keeps CI hermetic and offline. | DESIGN Reuse Analysis (oracle harness); DDD-10 |
